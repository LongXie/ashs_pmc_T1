...
# Ready to roll!
# perform super resolution upsampling
SRT1_FILE=$WORKDIR/T1_SR.nii.gz
SR $T1_FILE $SRT1_FILE

# run ASHS
$ASHS_ROOT/bin/ashs_main.sh \
  -a $ASHS_ATLAS \
  -g $T1_FILE -f $SRT1_FILE \
  -w $WORKDIR/ashs \
  -I $IDSTRING \
...
itksnap-wt  \
  -layers-set-main $SRT1_FILE -psn "Super-resolution upsampled T1 scan" \
